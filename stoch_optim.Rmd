...
We use the ucminf optimiser to find minima starting from the exact same points as the previous implementation did using optim(). 
...
We can use optimx() to run multiple optimisers (as in optim()), but using a much smaller number of lines of code and compare them.
